...
-- 数据清洗转换
INSERT OVERWRITE TABLE dwd_page_visit_detail PARTITION(dt='20250804')
SELECT 
    log.log_id,
    log.user_id,
    log.session_id,
...
LEFT JOIN ods_shop_page_info page 
    ON log.page_id = page.page_id AND page.ds='20250804'
LEFT JOIN ods_shop_page_info ref_page 
    ON log.referer_page_id = ref_page.page_id AND ref_page.ds='20250804'
WHERE log.ds='20250804';
...
-- 会话路径处理
INSERT OVERWRITE TABLE dwd_session_path PARTITION(dt='20250804')
SELECT
    session_id,
    user_id,
    entry_page_id,
...
-- 页面跳转关系处理
INSERT OVERWRITE TABLE dwd_page_jump PARTITION(dt='20250804')
SELECT
...
-- 转化数据处理
INSERT OVERWRITE TABLE dwd_user_conversion PARTITION(dt='20250804')
SELECT
    s.user_id,
    s.session_id,
