...
PARTITIONED BY (ds STRING)  -- 分区字段
STORED AS ORC
LOCATION '/warehouse/gd_gmall/dim/dim_user_info/';
--拉链更新 SQL（ds=2025-07-31）
INSERT OVERWRITE TABLE dim_user_info PARTITION (ds = '2025-07-31')
-- 新增或变更的当前有效记录
SELECT
    user_id,
    user_name,
    ...
    city,
    phone_prefix,
    '20250804' AS start_date,
    '20250804' AS end_date,
    1 AS is_current
FROM ods_user_info
WHERE ds = '20250804'  -- 仅处理当前日期的ODS数据
UNION ALL
...
SELECT
    ...
    a.phone_prefix,
    a.start_date,
    '20250804' AS end_date,  -- 失效日为当前日期前一天
    0 AS is_current
FROM dim_user_info a
JOIN ods_user_info b
ON a.user_id = b.user_id AND b.ds = '20250804'
WHERE a.ds = '20250804' AND a.is_current = 1  -- 关联当前分区的历史有效记录
...
--拉链更新 SQL（ds=2025-07-31）
INSERT OVERWRITE TABLE dim_product_info PARTITION (ds = '20230805')
-- 新增或变更的当前有效记录
SELECT
    product_id,
    product_name,
    ...
    status,
    create_time,
    '20230805' AS start_date,
    '9999-12-31' AS end_date,
    1 AS is_current
FROM ods_product_info
WHERE ds = '20230805'  -- 仅处理当前日期的ODS数据
UNION ALL
-- 历史有效记录中发生变更的旧记录（标记失效）
SELECT
    a.product_id,
...
--拉链更新 SQL（ds=20250804）
INSERT OVERWRITE TABLE dim_shop_page_info PARTITION (ds = '20240804')
-- 新增或变更的当前有效记录
SELECT
    page_id,
    page_name,
    ...
    is_active,
    create_time,
    '20240804' AS start_date,
    '9999-12-31' AS end_date,
    1 AS is_current
FROM ods_shop_page_info
WHERE ds = '20240804'  -- 仅处理当前日期的ODS数据
UNION ALL
-- 历史有效记录中发生变更的旧记录（标记失效）
SELECT
    a.page_id,
    ...
    a.create_time,
    a.start_date,
    '20240804' AS end_date,
    0 AS is_current
FROM dim_shop_page_info a
JOIN ods_shop_page_info b
ON a.page_id = b.page_id AND b.ds = '20240804'
WHERE a.ds = '20240804' AND a.is_current = 1
...
--拉链更新 SQL（ds=20250804）
INSERT OVERWRITE TABLE dim_traffic_source PARTITION (ds = '20250804')
-- 新增或变更的当前有效记录
SELECT
    source_id,
    source_type,
...
ON a.source_id = b.source_id AND b.ds = '20250804'
WHERE a.ds = '20250804' AND a.is_current = 1
...
--拉链更新 SQL（ds=20250804）
INSERT OVERWRITE TABLE dim_marketing_activity PARTITION (ds = '20250804')
-- 新增或变更的当前有效记录
SELECT
    activity_id,
    activity_name,
...
ON a.activity_id = b.activity_id AND b.ds = '20250804'
WHERE a.ds = '20250804' AND a.is_current = 1
...
--拉链更新 SQL（ds=20250804）
INSERT OVERWRITE TABLE dim_page_relationship PARTITION (ds = '20250701')
-- 新增或变更的当前有效记录
SELECT
    from_page_id,
    to_page_id,
...
WHERE ds = '20250701'  -- 仅处理当前日期的ODS数据
UNION ALL
-- 历史有效记录中发生变更的旧记录（标记失效）
SELECT
    a.from_page_id,
...
ON a.from_page_id = b.from_page_id AND a.to_page_id = b.to_page_id AND b.ds = '20250804'
WHERE a.ds = '20250804' AND a.is_current = 1
