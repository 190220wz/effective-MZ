...
INSERT OVERWRITE TABLE dws_device_traffic_stats_1d PARTITION(dt='20250804')
SELECT
    sp.device_type,
...
LEFT JOIN dwd_user_conversion uc
    ON sp.session_id = uc.session_id AND uc.dt='20250804'
WHERE sp.dt='20250804'
...
INSERT INTO TABLE dws_device_traffic_stats_7d PARTITION(dt='20250804')
SELECT
    sp.device_type,
    COUNT(sp.session_id) AS session_count,
    COUNT(DISTINCT sp.user_id) AS visitor_count,
...
FROM (
    ...
    WHERE dt BETWEEN '2025-07-25' AND '20250804'
) sp
LEFT JOIN (
...
INSERT INTO TABLE dws_device_traffic_stats_30d PARTITION(dt='20250804')
SELECT
    sp.device_type,
    COUNT(sp.session_id) AS session_count,
    COUNT(DISTINCT sp.user_id) AS visitor_count,
...
FROM (
    SELECT * FROM dwd_session_path
    WHERE dt BETWEEN '2025-07-02' AND '20250804'
) sp
LEFT JOIN (
    SELECT * FROM dwd_user_conversion
    WHERE dt BETWEEN '2025-07-02' AND '20250804'
...
INSERT OVERWRITE TABLE dws_page_visit_stats_1d PARTITION(dt='20250804')
SELECT
    pvd.page_id,
    pvd.page_name,
    pvd.page_type,
...
JOIN dwd_session_path sp
    ON pvd.session_id = sp.session_id AND sp.dt='20250804'
WHERE pvd.dt='20250804'
...
INSERT INTO TABLE dws_page_visit_stats_7d PARTITION(dt='20250804')
SELECT
    pvd.page_id,
    pvd.page_name,
    pvd.page_type,
...
INSERT INTO TABLE dws_page_visit_stats_30d PARTITION(dt='20250804')
SELECT
    pvd.page_id,
    pvd.page_name,
    pvd.page_type,
...
INSERT OVERWRITE TABLE dws_traffic_entry_stats_1d PARTITION(dt='20250804')
SELECT
    sp.entry_page_id,
...
JOIN dwd_page_visit_detail pvd
    ON sp.entry_page_id = pvd.page_id AND pvd.dt='20250804'
WHERE sp.dt='20250804'
...
INSERT INTO TABLE dws_traffic_entry_stats_7d PARTITION(dt='20250804')
SELECT
    sp.entry_page_id,
    pvd.page_name AS entry_page_name,
    pvd.page_type AS entry_page_type,
...
INSERT INTO TABLE dws_traffic_entry_stats_30d PARTITION(dt='20250804')
SELECT
    sp.entry_page_id,
    pvd.page_name AS entry_page_name,
    pvd.page_type AS entry_page_type,
...
INSERT OVERWRITE TABLE dws_page_path_flow_1d PARTITION(dt='20250804')
SELECT
  pj.from_page_id,
...
INSERT OVERWRITE TABLE dws_page_path_flow_7d PARTITION(dt='20250804')
SELECT
    pj.from_page_id,
...
INSERT OVERWRITE TABLE dws_page_path_flow_30d PARTITION(dt='20250804')
SELECT
    pj.from_page_id,
    MAX(f_pvd.page_name) AS from_page_name,
    MAX(f_pvd.page_type) AS from_page_type,
...
INSERT OVERWRITE TABLE dws_user_path_behavior_1d PARTITION(dt='20250804')
SELECT
    sp.user_id,
...
INSERT OVERWRITE TABLE dws_user_path_behavior_7d PARTITION(dt='20250804')
SELECT
    sp.user_id,
    COUNT(DISTINCT sp.session_id) AS session_count,
    SUM(sp.path_depth) AS total_page_views,
...
LEFT JOIN dwd_user_conversion uc
    ON sp.session_id = uc.session_id
    AND uc.dt BETWEEN '2025-07-25' AND '20250804'
JOIN (
    SELECT
        session_id,
        page_name AS entry_page,
...
INSERT OVERWRITE TABLE dws_user_path_behavior_30d PARTITION(dt='20250804')
SELECT
    sp.user_id,
    COUNT(DISTINCT sp.session_id) AS session_count,
    SUM(sp.path_depth) AS total_page_views,
...
LEFT JOIN dwd_user_conversion uc
    ON sp.session_id = uc.session_id
    AND uc.dt BETWEEN '2025-07-02' AND '20250804'
JOIN (
    SELECT
        session_id,
        page_name AS entry_page,
...
INSERT OVERWRITE TABLE dws_page_type_summary_1d PARTITION(dt='20250804')
SELECT
    pvd.page_type,
    pvd.device_type,
...
JOIN dwd_session_path sp
    ON pvd.session_id = sp.session_id AND sp.dt = '20250804'
LEFT JOIN dwd_user_conversion uc
    ON pvd.session_id = uc.session_id AND uc.dt = '20250804'
GROUP BY
    pvd.page_type,
    pvd.device_type;
...
INSERT OVERWRITE TABLE dws_page_type_summary_7d PARTITION(dt='20250804')
SELECT
    pvd.page_type,
    pvd.device_type,
    COUNT(*) AS visit_count,
...
FROM (
    ...
    FROM dwd_page_visit_detail
    WHERE dt BETWEEN DATE_SUB('20250804', 6) AND '20250804'  -- 动态计算7天范围
) pvd
JOIN (
    SELECT session_id, COUNT(*) AS page_count
    FROM dwd_page_visit_detail
    WHERE dt BETWEEN DATE_SUB('20250804', 6) AND '20250804'
    GROUP BY session_id
) sp ON pvd.session_id = sp.session_id
LEFT JOIN dwd_user_conversion uc
    ON pvd.session_id = uc.session_id
    AND uc.dt BETWEEN DATE_SUB('20250804', 6) AND '20250804'
...
INSERT OVERWRITE TABLE dws_page_type_summary_30d PARTITION(dt='20250804')
SELECT
    pvd.page_type,
    pvd.device_type,
    COUNT(*) AS visit_count,
...
FROM (
    SELECT
        *,
        ROW_NUMBER() OVER (PARTITION BY session_id ORDER BY visit_time DESC) = 1 AS is_last_page
    FROM dwd_page_visit_detail
    WHERE dt BETWEEN DATE_SUB('20250804', 29) AND '20250804'  -- 动态计算30天范围[10](@ref)
) pvd
JOIN (
    -- 计算每个会话的总页面数（用于跳出率判断）
    SELECT
        ...
        COUNT(*) AS page_count
    FROM dwd_page_visit_detail
    WHERE dt BETWEEN DATE_SUB('20250804', 29) AND '20250804'
    GROUP BY session_id
) sp ON pvd.session_id = sp.session_id
LEFT JOIN dwd_user_conversion uc  -- 关联用户转化表
    ON pvd.session_id = uc.session_id
    AND uc.dt BETWEEN DATE_SUB('20250804', 29) AND '20250804'
GROUP BY
    pvd.page_type,
    pvd.device_type;
