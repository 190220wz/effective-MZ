...
SELECT
    ...
    CASE WHEN conversion_rate > 1.0 THEN 1.0 ELSE ROUND(conversion_rate, 4) END,
    dt
FROM dws_device_traffic_stats_7d
WHERE dt = '20250804'
...
SELECT
    ...
    CASE WHEN conversion_rate > 1.0 THEN 1.0 ELSE ROUND(conversion_rate, 4) END,
    dt
FROM dws_device_traffic_stats_30d
WHERE dt = '20250804'
AND device_type IS NOT NULL;
...
SELECT
    '1d' AS period_type,
    page_id,
    COALESCE(page_name, '未知页面') AS page_name,
    COALESCE(page_type, '其他') AS page_type,
...
SELECT
    ...
    entry_count,
    dt
FROM dws_page_visit_stats_7d  -- 修正：添加空格，表名正确解析
WHERE dt = '20250804'
...
SELECT
    ...
    CASE WHEN conversion_rate > 1.0 THEN 1.0 ELSE ROUND(conversion_rate, 4) END,
    dt
FROM dws_traffic_entry_stats_1d
WHERE dt = '20250804'
...
SELECT
    ...
    CASE WHEN conversion_rate > 1.0 THEN 1.0 ELSE ROUND(conversion_rate, 4) END,
    dt
FROM dws_traffic_entry_stats_7d
WHERE dt = '20250804'
...
SELECT
    ...
    CASE WHEN conversion_rate > 1.0 THEN 1.0 ELSE ROUND(conversion_rate, 4) END,
    dt
FROM dws_traffic_entry_stats_30d
WHERE dt = '20250804';
...
SELECT
    ...
    jump_count,
    jump_users,
    CASE WHEN jump_rate > 1.0 THEN 1.0 ELSE ROUND(jump_rate, 4) END AS jump_rate,
    drop_off_count,
    dt
FROM dws_page_path_flow_1d
WHERE dt = '20250804'
...
SELECT
    ...
    drop_off_count,
    dt
FROM dws_page_path_flow_30d  -- 修正表名，使用30天周期的页面路径表
WHERE dt = '20250804';
