...
def generate_hive_ddl(table, columns, host, port, output_dir="ddl_scripts"):
    ...
    hive_ddl += f"use ds_path;\n"
    hive_ddl += f"-- 源表: ds_path.{table}\n"
    ...
    # 添加存储格式和压缩
    hive_ddl += "PARTITIONED BY (dt STRING)\n"
    hive_ddl += f"LOCATION 'hdfs://{host}:{port}/warehouse/tms/ods/ods_{table}'\n"
    hive_ddl += """
    TBLPROPERTIES (
    'orc.compress' = 'SNAPPY',
...
def generate_seatunnel_config(mysql_host, mysql_port, mysql_user, mysql_password,
    ...
    # 添加dt字段到列名列表
    column_names_with_dt = column_names + ["dt"]
    ...
    if custom_dt:
        # 使用自定义日期
        print(f"  → 使用自定义日期: {custom_dt}")
        if has_create_time:
            query = f"SELECT {columns_list},'{custom_dt}' as dt FROM `{database}`.`{table}`"
        else:
            query = f"SELECT {columns_list},'{custom_dt}' as dt FROM `{database}`.`{table}`"
    else:
        # 原有逻辑
        if has_create_time:
            ...
            query = f"SELECT {columns_list},date_format(create_time,'%Y%m%d') as dt FROM `{database}`.`{table}`"
        else:
            query = f"SELECT {columns_list},date_format(now(),'%Y%m%d') as dt FROM `{database}`.`{table}`"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dt', default=None, help='自定义分区日期 (格式: YYYYMMDD)')  # 新增参数
    ...
    custom_dt = args.dt  # 获取自定义日期参数

    # 获取所有表名
    print("正在获取tms01数据库表列表...")
    ...
    # python mysql_hive.py --dt 2025-07-31