...
set hive.vectorized.execution.enabled = false;
insert overwrite table dws_trade_user_sku_order_1d partition(dt='20250625')
select
    user_id,
    id,
    sku_name,
...
from gmall.dwd_trade_order_detail_inc
group by user_id,dt;
insert overwrite table dws_trade_user_order_1d partition(dt='20250627')
select
    user_id,
    count(distinct(order_id)),
    sum(sku_num),
...
group by user_id,dt;
insert overwrite table dws_trade_user_cart_add_1d partition(dt='20250627')
select
    user_id,
    count(*),
    sum(sku_num)
...
insert overwrite table dws_trade_user_cart_add_1d partition(dt='20250625')
select
    user_id,
    count(*),
    sum(sku_num)
...
group by user_id,dt;
insert overwrite table dws_trade_user_payment_1d partition(dt='20250627')
select
    user_id,
    count(distinct(order_id)),
    sum(sku_num),
...
insert into table dws_trade_user_payment_1d partition(dt='20250625')
select
    user_id,
    count(distinct(order_id)),
    sum(sku_num),
...
insert into table dws_trade_province_order_1d partition(dt='20250625')
select
    province_id,
    province_name,
    area_code,
...
insert overwrite table dws_tool_user_coupon_coupon_used_1d partition(dt='20250625')
select
    user_id,
    coupon_id,
    coupon_name,
...
insert overwrite table dws_interaction_sku_favor_add_1d partition(dt='20250625')
select
    sku_id,
    sku_name,
    category1_id,
...
(
    ...
    TBLPROPERTIES ('orc.compress' = 'snappy');
insert overwrite table dws_traffic_session_page_view_1d partition(dt='20250627')
select
    session_id,
    mid_id,
    brand,
...
insert into table dws_traffic_session_page_view_1d partition(dt='20250625')
select
    session_id,
    mid_id,
    brand,
...
(
    ...
    TBLPROPERTIES ('orc.compress' = 'snappy');
insert overwrite table dws_traffic_page_visitor_page_view_1d partition(dt='20250627')
select
    mid_id,
    brand,
    model,
...
(
    ...
    TBLPROPERTIES ('orc.compress' = 'snappy');
insert overwrite table dws_trade_user_sku_order_nd partition(dt='20250627')
select
    user_id,
    sku_id,
    sku_name,
    ...
    tm_id,
    tm_name,
    sum(if(dt>=date_add('20250627',-6),order_count_1d,0)),
    sum(if(dt>=date_add('20250627',-6),order_num_1d,0)),
    sum(if(dt>=date_add('20250627',-6),order_original_amount_1d,0)),
    sum(if(dt>=date_add('20250627',-6),activity_reduce_amount_1d,0)),
    sum(if(dt>=date_add('20250627',-6),coupon_reduce_amount_1d,0)),
    sum(if(dt>=date_add('20250627',-6),order_total_amount_1d,0)),
    sum(order_count_1d),
    sum(order_num_1d),
...
(
    ...
    TBLPROPERTIES ('orc.compress' = 'snappy');
insert overwrite table dws_trade_province_order_nd partition(dt='20250627')
select
    province_id,
    province_name,
    area_code,
...
(
    ...
    TBLPROPERTIES ('orc.compress' = 'snappy');
insert overwrite table dws_trade_user_order_td partition(dt='20250627')
select
    user_id,
...
select nvl(old.user_id, new.user_id),
       if(old.user_id is not null, old.order_date_first, '20250627'),
       if(new.user_id is not null, '20250627', old.order_date_last),
...
(
    ...
    TBLPROPERTIES ('orc.compress' = 'snappy');
insert overwrite table dws_user_user_login_td partition (dt = '20250627')
select nvl(old.user_id, new.user_id)                             as           user_id,
       if(new.user_id is null, old.login_date_last, '20250627')      as     login_date_last,
       if(old.login_date_first is null, '20250625', old.login_date_first) as login_date_first,
