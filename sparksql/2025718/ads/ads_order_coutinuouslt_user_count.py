...
def select_to_hive(df, tableName, partition_date):
    ...
    df.write.mode('append').insertInto(f"gmall.{tableName}")
...
def execute_hive_insert(partition_date: str, tableName):
    ...
    query1 = """
    SELECT '2025-06-27' AS dt, 7 AS recent_days, COUNT(DISTINCT user_id) AS order_continuously_user_count
    FROM (
        ...
        WHERE dt >= DATE_ADD('2025-06-27', -6)
    ) t1
    WHERE diff = 2;
    """
    ...
    query3 = """
    SELECT '2025-06-27' AS dt, 7 AS recent_days, COUNT(*) AS order_continuously_user_count
    FROM (
